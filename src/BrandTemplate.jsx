  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "#737373" }}>
          {color.hex}
        </div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#525252", marginTop: 2 }}>
          {color.token}
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div style={{ width: 160, flexShrink: 0 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#737373", marginBottom: 2 }}>
          {name}
        </div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#525252" }}>
          {size} / {weight}
        </div>
      </div>
      <div style={{ fontFamily: font, fontSize: parseInt(size), fontWeight: weight, color, lineHeight: 1.3 }}>
        {sample}
      </div>
    </div>
  );
...
function GradientShowcase() {
  const gradients = [
    { name: "Full Spectrum", css: `linear-gradient(90deg, ${COLORS.map((c) => c.hex).join(", ")})` },
    ...
    { name: "Radial Burst", css: "radial-gradient(circle at 30% 50%, #FF2255, #8844FF, #00D4FF)" },
  ];
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
      {gradients.map((g) => (
        <div key={g.name}>
          <div style={{ height: 80, borderRadius: 12, background: g.css, marginBottom: 10 }} />
          <div
            style={{
              fontFamily: "'Space Grotesk', sans-serif",
              fontSize: 14,
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        {COLORS.slice(0, 3).map((c, i) => (
          ...
          >
            ...
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#737373" }}>
  ...
  return (
    <div style={{ background: "#171717", border: "1px solid #262626", borderRadius: 12, overflow: "hidden" }}>
      ...
      {tokens.map((t) => (
        ...
        >
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "#d4d4d4" }}>
            {t.token}
          </span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "#737373" }}>
            {t.value}
          </span>
          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#737373" }}>{t.usage}</span>
          <div
            style={{ width: 20, height: 20, borderRadius: 4, background: t.color, border: "1px solid #404040" }}
          />
        </div>
      ))}
    </div>
...
function AccessibilityGrid() {
  const combos = [
    { bg: "#0a0a0a", fg: "#f5f5f5", label: "White on Black", ratio: "19.3:1", pass: true },
    { bg: "#0a0a0a", fg: "#FF6B2B", label: "Ember on Black", ratio: "5.2:1", pass: true },
    { bg: "#0a0a0a", fg: "#00D4FF", label: "Arc on Black", ratio: "10.1:1", pass: true },
    { bg: "#0a0a0a", fg: "#8844FF", label: "Drift on Black", ratio: "3.6:1", pass: false },
    { bg: "#171717", fg: "#FF2255", label: "Fuse on Surface", ratio: "4.3:1", pass: false },
    { bg: "#171717", fg: "#4488FF", label: "Signal on Surface", ratio: "4.8:1", pass: true },
  ];
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12 }}>
      {combos.map((c) => (
        <div
          key={c.label}
          style={{ background: c.bg, border: "1px solid #262626", borderRadius: 10, padding: 20 }}
        >
          <div
            style={{
              fontFamily: "'Space Grotesk', sans-serif",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, color: "#737373" }}>{c.label}</span>
            <span
              style={{
                fontFamily: "'JetBrains Mono', monospace",
                fontSize: 11,
                fontWeight: 500,
                color: c.pass ? "#a3a3a3" : "#737373",
                background: c.pass ? "#ffffff10" : "#ffffff08",
                padding: "3px 8px",
                borderRadius: 4,
              }}
            >
...
export default function BrandTemplate() {
  ...
  return (
    <>
      ...
      <div style={{ background: "#0a0a0a", minHeight: "100vh", color: "#f5f5f5", fontFamily: "'Inter', sans-serif" }}>
        <header style={{ padding: "40px 48px 0" }}>
          ...
          >
            <div>
              ...
              <p style={{ fontSize: 15, color: "#737373", maxWidth: 420, lineHeight: 1.6 }}>
                A six-color accent spectrum built on a grayscale foundation. Warm to cool, ember to arc.
              </p>
            </div>
            <div style={{ display: "flex", gap: 6, paddingTop: 8 }}>
              {COLORS.map((c) => (
                <div key={c.hex} style={{ width: 12, height: 48, borderRadius: 6, background: c.hex }} />
              ))}
            </div>
          </div>
          ...
          <nav style={{ display: "flex", gap: 4, paddingTop: 20 }}>
            {nav.map((item) => (
              <button
                key={item.id}
                onClick={() => {
        ...
        <main style={{ padding: "48px 48px 96px" }}>
          ...
          >
            <div style={{ background: "#171717", border: "1px solid #262626", borderRadius: 12, padding: "8px 24px" }}>
              <TypographyRow
          ...
          <Section id="a11y" title="Accessibility" subtitle="Contrast ratios for accent colors on primary backgrounds.">
            <AccessibilityGrid />
          </Section>
          ...
          <div style={{ marginTop: 80 }}>
            ...
            >
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#404040" }}>
                BlackRoad Design System · Brand Template v1.0
              </span>
              <div style={{ display: "flex", gap: 3 }}>
                {COLORS.map((c) => (
                  <div key={c.hex} style={{ width: 8, height: 8, borderRadius: "50%", background: c.hex }} />
                ))}
              </div>
            </div>
          </div>
